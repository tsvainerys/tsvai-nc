...
let blogName = "T'Svai's Blog";
let authorName = "T'Svai";
...
[ "posts/2025-02-27-chugging-along.html", "", "updates", "paracosm"],
...
let footerHTML = "<hr><p>" + "blog written by <a href='" + authorLink + "'>" + authorName + "</a>, built with <a href='https://zonelets.net/'>Zonelets</a>, tagging system by <a href='https://3legged.neocities.org/journal/posts/2023-03-12-Creating-A-Zonelets-Tagging-System'>3legged</a> <3 </p>";
  ...
  if (  postDateFormat.test ( postsArray[currentIndex][0].slice( 6,17 ) ) ) {
	...
	for (i=2;i<postsArray[currentIndex].length;i++) {
	  console.log(postsArray[currentIndex][i]);
		postTagsHTML += '<a href="../tags/' + postsArray[currentIndex][i] + '.html">' + postsArray[currentIndex][i] + '</a>, ';
	}
...
if ( postsArray.length < 2 ) {
  nextprevHTML = '<a href="' + relativePath + '/index.html">index</a>';
} else if ( currentIndex === 0 ) {
  prevlink = postsArray[currentIndex + 1][0];
  nextprevHTML = '<a href="' + relativePath + '/index.html">index</a> | <a href="'+ relativePath + '/' + prevlink +'">Previous Post \u00BB</a>';
} else if ( currentIndex === postsArray.length - 1 ) {
  nextlink = postsArray[currentIndex - 1][0];
  nextprevHTML = '<a href="' + relativePath + '/' + nextlink +'">\u00AB Next Post</a> | <a href="' + relativePath + '/index.html">index</a>';
...
function formatTagList (tagArray) {
	...
	for (i=0;i<tagArray.length;i++) {
  		tagListHTML += '<li><a href="tags/' + tagArray[i] + '.html">' + tagArray[i] + '</a></li>';
	}
	tagListHTML += "</ul>";
	return tagListHTML;
}
